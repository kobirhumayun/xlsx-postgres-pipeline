...
export default function ImportPage() {
  ...
  useEffect(() => {
    ...
    if (dbToCheck) {
      fetchJson(`/api/structure?database=${dbToCheck}`).then(data => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Enforce header row is always true (backend should also enforce this or default to true)
      // The user requirement says "Header Row must always be included".
      // We can append a flag if the backend requires it, or just rely on backend defaults.
      // Assuming backend handles it or we'll update backend next.
      payload.append("hasHeader", "true");
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-8">
        <header className="space-y-3">
          ...
          <p className="text-base text-zinc-600">
            Import Excel data into flexible table locations. Header row is required in the Excel file.
          </p>
        </header>
